...
uvec3 rotateOffset(uvec3 offset)
{
	switch ( uWorldRotation % 4 )
	{
		...
		case 1: return uvec3( offset.y, -offset.x, offset.z );
		case 2: return uvec3( -offset.x, -offset.y, offset.z );
		case 3: return uvec3( -offset.y, offset.x, offset.z );
	}
}
...
void main()
{
	...
	const bool uIsWall = a_position.z != 0.0;

	// Min and max are inclusive in volume
	const uvec3 renderVolume = uRenderMax - uRenderMin + uvec3(1u, 1u, 1u);
	...
	uint vFlags = tileData[index].flags;
	uint vFlags2 = tileData[index].flags2;
	uint vLightLevel = ( tileData[index].packedLevels >> 8 ) & 0xff;
	uint vVegetationLevel = ( tileData[index].packedLevels >> 16 ) & 0xff;
	...
	if ( uIsWall )
	{
		containsTransparency = ( vFlags & TF_TRANSPARENT ) != 0 || ( tileData[index].packedLevels & 0xff ) >= 3;
	}
	else
	{
		containsTransparency = ( vFlags & TF_TRANSPARENT ) != 0 || ( tileData[index].packedLevels & 0xff ) != 0;
	}
	const bool renderingEnabled = uPaintFrontToBack ^^ containsTransparency;
	if ( renderingEnabled )
	{
		floorSprite = tileData[index].floorSpriteUID;
		wallSprite = tileData[index].wallSpriteUID;
		itemSprite = tileData[index].itemSpriteUID;
		jobFloorSprite = tileData[index].jobSpriteFloorUID;
		jobWallSprite = tileData[index].jobSpriteWallUID;
		creatureSprite = tileData[index].creatureSpriteUID;
	}
	...
	if ( !uPaintFrontToBack )
	{
		...
		const uint vFluidLevel = tileData[index].packedLevels & 0xff;

		const uint vFluidLevelAbove = tileData[indexAbove].packedLevels & 0xff;
		const uint vFluidLevelLeft = tileData[indexL].packedLevels & 0xff;
		const uint vFluidLevelRight = tileData[indexR].packedLevels & 0xff;
		...
		if ( vFluidLevel >= 1 )
		{
			if (vFluidLevel <= 2)
			{
				...
				if(tileData[index].floorSpriteUID != 0)
				{
	...
	else
	{
		...
		if( ( vFlags & TF_OVERSIZE ) != 0 )
		{
			...
			vVertexCoords.x = floor(a_position.x + 0.5);
			vVertexCoords.y = floor(a_position.y + 0.5);
		}
		else
		{
			vVertexCoords = a_position.xy;
		}

		v_texcoord0 = vec2( vVertexCoords.x, 1.0 - vVertexCoords.y );
		v_texcoord1 = uvec4(floorSprite, jobFloorSprite, wallSprite, jobWallSprite);
		v_texcoord2 = uvec4(itemSprite, creatureSprite, vFluidLevelPacked1, uIsWall);
