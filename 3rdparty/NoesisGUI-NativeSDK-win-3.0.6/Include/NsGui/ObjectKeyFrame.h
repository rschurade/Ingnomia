...
public:
    ...
    Ptr<BaseComponent> InterpolateValue(const Ptr<BaseComponent>& baseValue, float keyFrameProgress);
...
protected:
    virtual Ptr<BaseComponent> InterpolateValueCore(const Ptr<BaseComponent>&, float keyFrameProgress) = 0;
